...
{
	return level; //wc - only one level per world
...
{
	return world - 1; //wc - only one level per world
	if (world == 1)
		return LV_JUNGLE - 1;
...
long LevelStats(long level)
{
	...
	CreateSaveGameInfo();
	S_SaveGame(&savegame, sizeof(SAVEGAME_INFO), 30 + level);
	s = savegame.timer / 30;
	...
	if (tomb3.gold /*|| !gameflow.globe_enabled wc - I want the globe every time*/)
	{
#if (DIRECT3D_VERSION >= 0x900)
		if (!bMonoScreen)
			FadePictureDown(32);
	...
	//wc - game plus mode enabled from level 5 (end level)
	else if (level == 5 && !savegame.bonus_flag)
	{
		savegame.bonus_flag = 1;
...
long StartGame(long level, long type)
{
	...
	//wc - don't modify inventory if returning to a level
	if (type != 2 && !((savegame.IndiaComplete && CurrentLevel == 1)
