...
void do_levelselect_option(INVENTORY_ITEM* item)
{
	...
	{
		//wc - running into naming issues, so just display the level name instead of the globe name
		char Adventure[6] = { 1, 2, 3, 1, 4, 5 }; //wc - only one level per world	//starting levels for: india, pacific, nevada, peru, london, antarc.
		int selectedLevel = Adventure[GlobeLevel == 5 ? 4 : GlobeLevel == 4 ? 5 : GlobeLevel];
		AdventureText = T_Print(0, -16, 5, GF_Level_Names[selectedLevel]);
	...
	if ((inputDB & (IN_SELECT | IN_DESELECT)) && GlobeLevel != 6)
	{
		...
		//wc - world 5 is level 4 and vice versa
		if (GlobeLevel == 5)
			NextAdventure = 4;
		else
		if (GlobeLevel == 4)
...
{
	if (CurrentLevel == 4) //wc - notebook hint
	{
		S_Pause(10);
		item->anim_direction = 1;
		item->goal_frame = item->frames_total - 1;
	}
	else if (inputDB & (IN_SELECT | IN_DESELECT))
	{
		item->anim_direction = 1;
		item->goal_frame = item->frames_total - 1;
	}
...
void do_grappling_hook_option(INVENTORY_ITEM* item)
{
	if (inputDB & (IN_SELECT | IN_DESELECT))
	{
		if (inputDB & IN_SELECT)
		{
			...
			FLOOR_INFO* floor = GetFloor(lara_item->pos.x_pos, lara_item->pos.y_pos, lara_item->pos.z_pos, &room_number);
			long height = GetHeight(floor, lara_item->pos.x_pos, lara_item->pos.y_pos, lara_item->pos.z_pos);
			long ceiling = GetCeiling(floor, lara_item->pos.x_pos, lara_item->pos.y_pos, lara_item->pos.z_pos);
			//wc - try to use grappling hook if standing still as human, with empty hands and at least 5 clicks above
			if (lara_item->current_anim_state != AS_STOP || lara.gun_status != LG_ARMLESS || ceiling - lara_item->pos.y_pos > -1280 || lara_item->ai_bits & 1)
			{
				SoundEffect(SFX_LARA_NO, 0, SFX_ALWAYS);
				return;
			}
			lara.gun_status = LG_HANDSBUSY;
			long origX, origZ, dist;
			origX = lara_item->pos.x_pos;
			origZ = lara_item->pos.z_pos;
			//start 1 sector away for a max search of 5 sectors
			dist = 1024;
			while (dist < 1024 * 5)
			{
				dist += 128;
				lara_item->pos.x_pos = origX + (dist * phd_sin(lara_item->pos.y_rot) >> W2V_SHIFT);
				lara_item->pos.z_pos = origZ + (dist * phd_cos(lara_item->pos.y_rot) >> W2V_SHIFT);
				floor = GetFloor(lara_item->pos.x_pos, lara_item->pos.y_pos - 1280, lara_item->pos.z_pos, &room_number);
				height = GetHeight(floor, lara_item->pos.x_pos, lara_item->pos.y_pos - 1280, lara_item->pos.z_pos);
				ceiling = GetCeiling(floor, lara_item->pos.x_pos, lara_item->pos.y_pos - 1280, lara_item->pos.z_pos);
				//obstacle in the way, stop checking
				if (height < lara_item->pos.y_pos + 512 - 1280 && height != NO_HEIGHT)
					break;
				if (ceiling > lara_item->pos.y_pos - 1280 && ceiling != NO_HEIGHT)
					break;
				//check for climb flags
				...
				{
					auto quad = ushort(lara_item->pos.y_rot + 8192) >> 14;
			...
			lara_item->pos.x_pos = origX;
			lara_item->pos.z_pos = origZ;
			if (lara_item->goal_anim_state != AS_ALL4S)
			{
				SoundEffect(SFX_LARA_NO, 0, SFX_ALWAYS);
				return;
			}
...
void do_fast_travel_option(INVENTORY_ITEM* item)
{
	if (inputDB & (IN_SELECT | IN_DESELECT))
	{
		if (inputDB & IN_SELECT)
		{
			//wc - fast travel to corresponding key lock if standing still and fast travel not locked
			if (lara_item->current_anim_state != AS_STOP || lara_item->ai_bits & 4)
			{
				SoundEffect(SFX_LARA_NO, 0, SFX_ALWAYS);
				return;
			}
			...
			for (int i = 0; i < level_items; i++)
			{
				...
				if (item2->object_number == item->object_number + 4) //key lock is 4 above the key item
				{
					lara_item->pos.x_pos = item2->pos.x_pos;
					lara_item->pos.y_pos = item2->pos.y_pos;
...
void do_inventory_options(INVENTORY_ITEM* item)
{
	...
	case KEY_OPTION3:
	case KEY_OPTION4:
		return do_fast_travel_option(item);
	...
	case PICKUP_OPTION2:
		return do_grappling_hook_option(item); //wc - pickup2 is the grappling hook
