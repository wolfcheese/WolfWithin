...
long S_Pause(short customText)
{
	...
	//wc - custom text display from flipeffect 38
	if (customText > 0)
	{
		TEXTSTRING* pauseStrings[10] = { nullptr };
		inputDB = GetDebouncedInput(input);
		do
		{
			...
			if (!pause_text)
			{
				...
				//txt[0] = 0;
				//wsprintf(txt, "%5d test!", customText);
				//pause_text = T_Print(0, -24, 5, txt);
				//T_CentreH(pause_text, 1);
				//T_CentreV(pause_text, 1);
				//T_AddBackground(pause_text, 304 * 4 - 12, 19, 0, 0, 16, 0, &req_sel_gour1, bgFlag);
				//T_AddOutline(pause_text, 1, 4, &req_sel_gour2, 0);

				pause_text = T_Print(0, -24, 5, GF_PCStrings[27 + customText]);
				//split by dash for multiple lines
				char* token;
				char stringToSplit[300];
				strcpy(stringToSplit, GF_PCStrings[27 + customText]);
				token = strtok(stringToSplit, "-");
				int height = 0;
				int pauseStringIndex = 0;
				while (token != NULL)
				{
					TEXTSTRING* pause_text2;
					pause_text2 = T_Print(0, height, 0, token);
					T_CentreH(pause_text2, 1);
					T_CentreV(pause_text2, 1);
					T_AddBackground(pause_text2, 304 * 4 - 12, 19, 0, 0, 16, 0, &req_sel_gour1, bgFlag);
					T_AddOutline(pause_text2, 1, 4, &req_sel_gour2, 0);
					pauseStrings[pauseStringIndex] = pause_text2;
					height += 50;
					pauseStringIndex++;
					token = strtok(NULL, "-");
				}
			...
			S_DumpScreen();
			inputDB = GetDebouncedInput(input);
			if (inputDB & IN_PAUSE || inputDB & IN_OPTION || inputDB & IN_SELECT)
				break;
		} while (!S_UpdateInput());
		//clear all pause strings
		for (int i = 0;i < 10;i++)
		{
			if (pauseStrings[i] != nullptr)
			{
				T_RemovePrint(pauseStrings[i]);
				pauseStrings[i] = nullptr;
			}
			else
				break;
		}
	...
	{
		vol = float(25 * Option_Music_Volume + 5);
