...
static void WillBossDie(short item_number)
{
	...
	//wc - hit heavy trigger when boss dies
...
void WillBossControl(short item_number)
{
	...
	{
		puzzle_complete = Inv_RequestItem(PUZZLE_ITEM1);
		puzzle_complete += Inv_RequestItem(PUZZLE_ITEM2);
		puzzle_complete += Inv_RequestItem(PUZZLE_ITEM3);
		puzzle_complete += Inv_RequestItem(PUZZLE_ITEM4);

		if (puzzle_complete == 0 && item->item_flags[1])
		{
