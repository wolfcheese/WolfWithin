...
void ControlHarpoonBolt(short item_number)
{
	...
	for (target_num = room[item->room_number].item_number; target_num != NO_ITEM; target_num = target->next_item)
	{
		...
		if ((obj_num == SMASH_OBJECT1 && CurrentLevel != 4 && CurrentLevel != 2) || obj_num == SMASH_WINDOW || obj_num == SMASH_OBJECT2 || obj_num == SMASH_OBJECT3)
		...
		else if (obj_num != SMASH_OBJECT1)
		{
			...
			{
				if (obj_num == TRIBEBOSS || obj_num == LON_BOSS)
				{
...
void ControlRocket(short item_number)
{
	...
	for (r = 0; r < number_draw_rooms; r++)
	{
		for (target_num = room[draw_rooms[r]].item_number; target_num != NO_ITEM; target_num = target->next_item)
		{
			...
			if (obj_num == SMASH_OBJECT1 && CurrentLevel == 4)
			{
			...
			else if ((obj_num == SMASH_OBJECT1 && CurrentLevel != 4 && CurrentLevel != 2) || obj_num == SMASH_WINDOW || obj_num == SMASH_OBJECT2 || obj_num == SMASH_OBJECT3)
				SmashWindow(target_num);
			else if (obj_num == CARCASS || obj_num == EXTRAFX6)
			{
				if (target->status != ITEM_ACTIVE)
			...
			{
				if ((obj_num == TRIBEBOSS || obj_num == LON_BOSS) && TribeBossShieldOn)
					FindClosestShieldPoint(item->pos.x_pos, item->pos.y_pos, item->pos.z_pos, target);
				else
				...
				{
					if (obj_num != TREX && obj_num != TRIBEBOSS && obj_num != WILLARD_BOSS && obj_num != TONY && obj_num != LON_BOSS &&
...
void ControlGrenade(short item_number)
{
	...
	for (r = 0; r < number_draw_rooms; r++)
	{
		for (target_num = room[draw_rooms[r]].item_number; target_num != NO_ITEM; target_num = target->next_item)
		{
			...
			if (obj_num == SMASH_OBJECT1 && CurrentLevel != 4 && CurrentLevel != 2)
				SmashWindow(target_num);
			...
			else if (obj_num != SMASH_OBJECT1)
			{
				...
				if (target->hit_points <= 0)
				{
					if (obj_num != TREX && obj_num != TRIBEBOSS && obj_num != WHALE && obj_num != WILLARD_BOSS && obj_num != TONY &&
