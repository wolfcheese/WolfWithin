...
TEXTSTRING* timerString = nullptr;
long ControlPhase(long nframes, long demo_mode)
{
	...
	for (framecount += nframes; framecount > 0; framecount -= TICKS_PER_FRAME)
	{
		...
		if (CurrentLevel == 4 && room[lara_item->room_number].flags & ROOM_OUTSIDE)
...
void TestTriggers(short* data, long heavy)
{
	...
	short type, flags, value, switchValue, trigger, camera_flags, camera_timer;
	char timer;
	...
	else
	{
		...
		case SWITCH:
			value = *data++ & 0x3FF;
			switchValue = value; //wc - for sequence switch puzzle
	...
	do
	{
		...
		case TO_OBJECT:
			...
			//wc - clear timer display and unlock fast travel if item is no longer timed
			if (timer < 1 && item->timer > 0 && timerString != nullptr)
			{
				if (lara_item->ai_bits & 4)
					lara_item->ai_bits ^= 4;
				T_RemovePrint(timerString);
			}

			item->timer = timer;

			//wc - hack for desert secret timed run (big timer doesn't work in trigger)
			if (item->timer > 0 && CurrentLevel == 3 && item->object_number == DOOR_TYPE1)
				item->timer += 100;

			if (timer != 1)
				item->timer *= 30;

			//wc - shiva shape puzzle only activates meshbits
			if (item->object_number == SHIVA)
			{
				switch (flags & IFL_CODEBITS)
				{
					case 12288: //bits 4 and 5
						item->mesh_bits ^= 64;
						break;
					case 8192: //bit 5
						item->mesh_bits ^= 32;
					...
					case 2048: //bit 3
						...
						break;
					case 1024: //bit 2
						item->mesh_bits ^= 6; //why does this work? it should be 4, but 4 renders the first two meshes for some reason
					case 512: //bit 1
						item->mesh_bits ^= 2;
						break;
					case IFL_CODEBITS:
				...
				//player has solved the puzzle (it matches the mirror image) so activate heavy trigger
				if (((flags & IFL_CODEBITS) != IFL_CODEBITS) && item->mesh_bits == 58)
				{
			...
			if (type == SWITCH)
			{
				//wc - camera target in desert teleports in and out of maze
				if (item->object_number == CAMERA_TARGET && CurrentLevel == 3 && item->room_number != lara_item->room_number)
				{
					if ((item->flags & IFL_CODEBITS) == IFL_CODEBITS)
					{
						...
						SoundEffect(SFX_LARA_NO, &lara_item->pos, SFX_DEFAULT);
					break;
				}

				//wc - rotation for laser puzzle in desert. reflectors rotate then don't process further
				if (CurrentLevel == 3 && item->object_number == MAP_CLOSED)
				{
					SoundEffect(SFX_DRILL_BIT_1, 0, SFX_ALWAYS);
					item->pos.y_rot -= 0x4000;
					if (item->pos.y_rot < -0x8000)
						item->pos.y_rot += 0x16000;
					break;
				}
				//wc - tundra valve puzzle lifts trapdoors depending on how many are active
				if (item->object_number == TRAPDOOR && CurrentLevel == 4 && items[switchValue].object_number == AIRLOCK_SWITCH)
				{
					SoundEffect(SFX_LAUNCHER_1, 0, SFX_ALWAYS);
					//check for current status of valves
					ITEM_INFO* switchItem;
					ITEM_INFO* trapdoorItems[3];
					int switchItemIndex = 0;
					int trapdoorItemIndex = 0;
					item->ai_bits = switch_off ? 2 : 1; //switch off/on is reversed for valves
					long yPos = -1;
					for (int i = 0; i < level_items; i++)
					{
						switchItem = &items[i];

						if (switchItem->object_number == AIRLOCK_SWITCH && switchItem->current_anim_state == 1)
						{
							switchItemIndex++;
							if (yPos == -1)
								yPos = switchItem->pos.y_pos;
						}
						else
							if (switchItem->object_number == TRAPDOOR && switchItem->ai_bits > 0)
							{
								trapdoorItems[trapdoorItemIndex++] = switchItem;
							}
						if (trapdoorItemIndex >= 3 && switchItemIndex >= 3)
							break;
					}
					for (int i = 0;i < trapdoorItemIndex;i++)
					{
						if(trapdoorItems[i]->ai_bits < 2)
							trapdoorItems[i]->pos.y_pos = yPos;
						else
						{
							switch (switchItemIndex)
							{
								case 0:
									trapdoorItems[i]->pos.y_pos = yPos;
									break;
								case 3:
									trapdoorItems[i]->pos.y_pos = yPos - 256 * 2;
									break;
								case 2:
									trapdoorItems[i]->pos.y_pos = yPos - 256 * 4;
									break;
								case 1:
									trapdoorItems[i]->pos.y_pos = yPos - 256 * 8;
									break;
							}
						}
					}
				...
				//wc - tundra banner puzzle scrolls between different anims
				if (item->object_number == ANIMATING4 && CurrentLevel == 4)
				{
					item->anim_number += 1;
					short baseAnim = objects[ANIMATING4].anim_index;
					if (item->anim_number > baseAnim + 4)
						item->anim_number = baseAnim;
					item->frame_number = anims[item->anim_number].frame_base;
					short roomNumber = item->room_number;
					//check for current status of banners
					bool puzzleSolved = true;
					for (int i = 0; i < level_items; i++)
					{
						item = &items[i];

						if (item->object_number == ANIMATING4)
						{
							switch (item->flags & IFL_CODEBITS)
							{
								case 8192: //bit 5
									if (item->anim_number != objects[ANIMATING4].anim_index + 5 - 1)
										puzzleSolved = false;
									break;
								case 4096: //bit 4
									if (item->anim_number != objects[ANIMATING4].anim_index + 4 - 1)
										puzzleSolved = false;
									break;
								case 2048: //bit 3
									if (item->anim_number != objects[ANIMATING4].anim_index + 3 - 1)
										puzzleSolved = false;
									break;
								case 1024: //bit 2
									if (item->anim_number != objects[ANIMATING4].anim_index + 2 - 1)
										puzzleSolved = false;
									break;
								case 512: //bit 1
									if (item->anim_number != objects[ANIMATING4].anim_index + 1 - 1)
										puzzleSolved = false;
									break;
							}
							if (!puzzleSolved)
					...
					if (puzzleSolved)
					{
						for (int i = 0; i < level_items; i++)
						{
							...
							if (item->object_number == CAMERA_TARGET && item->room_number == roomNumber)
							{
								short room_num = item->room_number;
								auto floor = GetFloor(item->pos.x_pos, item->pos.y_pos, item->pos.z_pos, &room_num);
								GetHeight(floor, item->pos.x_pos, item->pos.y_pos, item->pos.z_pos);
				...
				//wc - forest sequence switch puzzle uses ai_bits in doors to track progression
				if (item->object_number == DOOR_TYPE1 && CurrentLevel == LV_FIRSTLEVEL)
				{
					if (!switch_off)
					{
						//lock switch
						if (items[switchValue].status != ITEM_ACTIVE)
						{
							AddActiveItem(switchValue);
							items[switchValue].status = ITEM_ACTIVE;
							items[switchValue].ai_bits = 1;
						}
						//check for current status of sequence switches
						ITEM_INFO* switchItem;
						short switchItems[3];
						int switchItemIndex = 0;
						for (int i = 0; i < level_items; i++)
						{
							...
							if (switchItem->object_number == PUSH_SWITCH && switchItem->ai_bits > 0 && switchItem->current_anim_state == 0)
							{
								switchItems[switchItemIndex++] = i;
								if (switchItemIndex >= 3)
									break;
							}
						}
						if (switchItemIndex > 0)
						{
							if (item->ai_bits == 5 || (item->flags & IFL_CODEBITS) == 8192) //door bit 5 -> switch bits 5, 4, then 3
							{
								if (((flags & IFL_CODEBITS) == 8192) && (item->ai_bits == 0 || item->ai_bits == 1) && switchItemIndex == 1)
								{
									item->ai_bits = 1;
								}
								else if (((flags & IFL_CODEBITS) == 4096) && (item->ai_bits == 2 || item->ai_bits == 1) && switchItemIndex == 2)
								{
									item->ai_bits = 2;
								}
								else if (((flags & IFL_CODEBITS) == 2048) && (item->ai_bits == 2 || item->ai_bits == 5) && switchItemIndex == 3)
								{
									item->ai_bits = 5;
								}
								else {
							...
							else if (item->ai_bits == 4 || (item->flags & IFL_CODEBITS) == 4096) //door bit 4 -> switch bits 3, 4, then 5
							{
								if (((flags & IFL_CODEBITS) == 2048) && (item->ai_bits == 0 || item->ai_bits == 1) && switchItemIndex == 1)
								{
									item->ai_bits = 1;
								}
								else if (((flags & IFL_CODEBITS) == 4096) && (item->ai_bits == 2 || item->ai_bits == 1) && switchItemIndex == 2)
								...
								else if (((flags & IFL_CODEBITS) == 8192) && (item->ai_bits == 2 || item->ai_bits == 4) && switchItemIndex == 3)
								{
									item->ai_bits = 4;
								}
								else {
							...
							else if (item->ai_bits == 3 || (item->flags & IFL_CODEBITS) == 2048) //door bit 3 -> switch bits 5, 3, then 4
							{
								if (((flags & IFL_CODEBITS) == 8192) && (item->ai_bits == 0 || item->ai_bits == 1) && switchItemIndex == 1)
								{
									item->ai_bits = 1;
								}
								else if (((flags & IFL_CODEBITS) == 2048) && (item->ai_bits == 2 || item->ai_bits == 1) && switchItemIndex == 2)
								{
									item->ai_bits = 2;
								}
								else if (((flags & IFL_CODEBITS) == 4096) && (item->ai_bits == 2 || item->ai_bits == 3) && switchItemIndex == 3)
								{
									item->ai_bits = 3;
								}
								else {
							...
							//reset sequence switches if all three were used
							if (switchItemIndex >= 3)
							{
								for (int i = 0; i < 3; i++)
								{
									switchItem = &items[switchItems[i]];
									switchItem->goal_anim_state = 1;
								}
							}
						}
					}
		...
		case TO_FLIPEFFECT:
		{
			//wc - to prevent looping, use codebit approach that is used in audio
			//Using the flags for hardcoded tracks 0 and 1 (unused) for this purpose
			// skip flipeffect 48 and 51 for this check
			long code = (flags >> 8) & 0x3F;

			if (value != 48 && value != 51 && (cd_flags[value <= 43 || value >=53 ? 0 : 1] & code) == code)
				return;

			cd_flags[value <= 43 || value >= 53 ? 0 : 1] |= code;
			neweffect = value;
			break;
		}
...
long TriggerActive(ITEM_INFO* item)
{
	...
	if (!item->timer)
	{
		//wc - remove any times and unlock fast travel if it was locked
		if(lara_item->ai_bits & 4)
			lara_item->ai_bits ^= 4;
		if (timerString != nullptr)
			T_RemovePrint(timerString);
		item->timer = -1;
	}
	else if(item->object_number != SWITCH_TYPE1 && item->object_number != SWITCH_TYPE2 && item->object_number != PUSH_SWITCH)
	{
		if (timerString != nullptr)
			T_RemovePrint(timerString);
		//wc - display timer for timed items and lock fast travel
		char txt[32];
		short time = item->timer / 30;
		snprintf(txt, sizeof(txt), "%4d", time);
		timerString = T_Print(350, 48, 3, txt);
		lara_item->ai_bits |= 4;
	}
...
void TriggerNormalCDTrack(short value, short flags, short type)
{
	...
	if (value >= 26 && value <= 36 || value >= 73 && value <= 78 || value == 117 || value == 118 || value == 83)
	{
