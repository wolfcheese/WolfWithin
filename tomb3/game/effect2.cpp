...
static short previous_health;
static short previous_color;
...
void TriggerBlood(long x, long y, long z, long angle, long num)
{
	...
	long ang;
	short rad;
	bool isLara = previous_health > lara_item->hit_points || lara_item->hit_points <= 0; //wc - my attempt to track whether it was Lara that was just damaged
	previous_health = lara_item->hit_points;
	for (int i = 0; i < num; i++)
	{
		sptr = &sparks[GetFreeSpark()];
		sptr->On = 1;
		...
		//wc - custom enemy blood colors
		else if (CurrentLevel == 1 || (CurrentLevel == 5 && previous_color < 1))
		{
			previous_color = 1;
			sptr->sR = 0;
			sptr->sG = 171;
		...
		else if (CurrentLevel == 2 || (CurrentLevel == 5 && previous_color < 2))
		{
			previous_color = 2;
			sptr->sR = 204;
			sptr->sG = 209;
		...
		else if (CurrentLevel == 3 || (CurrentLevel == 5 && previous_color < 3))
		{
			previous_color = 3;
			sptr->sR = 211;
			sptr->sG = 8;
		...
		else if (CurrentLevel == 4 || (CurrentLevel == 5 && previous_color < 4))
		{
			previous_color = 0;
			sptr->sR = 43;
			sptr->sG = 0;
...
void TriggerBloodD(long x, long y, long z, long angle, long num)
{
	...
	long ang;
	short rad;
	bool isLara = previous_health > lara_item->hit_points || lara_item->hit_points <= 0;
	previous_health = lara_item->hit_points;
	for (int i = 0; i < num; i++)
	{
		sptr = &sparks[GetFreeSpark()];
