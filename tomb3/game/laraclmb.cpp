...
void lara_col_climbstnc(ITEM_INFO* item, COLL_INFO* coll)
{
	...
	if (input & (IN_FORWARD | IN_DUCK))
	{
		if (item->goal_anim_state == AS_NULL || item->goal_anim_state == AS_DUCK)
			return;
		...
		if (result_r && result_l)
		{
			...
			{
				//wc - allow to vault into crawl with duck key
				item->goal_anim_state = input & IN_DUCK ? AS_DUCK : AS_NULL;
				if(item->goal_anim_state == AS_DUCK)
		...
		{
			//wc - check for crawlspace overhead with hacky hang test. Doesn't work well on sloped crawlspaces
			auto oldYPos = item->pos.y_pos;
			item->pos.y_pos += 256;
			LaraHangTest(item, coll);
			if (coll->front_floor > -850 && coll->front_floor < -650 && coll->front_floor - coll->front_ceiling >= -256 &&
