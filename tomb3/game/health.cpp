...
void DrawBossHealthBar(long flash_state)
{
	static long old_bosshitpoints = 0;
	long bosshitpoints = 0;
	long maxhitpoints = 10;

	for (int i = 0; i < level_items; i++)
	{
		ITEM_INFO* item = &items[i];
		if (((item->object_number == TONY && CurrentLevel == 3) 
			|| (item->object_number == TREX && CurrentLevel == 2) 
			|| (item->object_number == LON_BOSS && CurrentLevel == 4) 
			|| (item->object_number == TRIBEBOSS && CurrentLevel == 1)
			|| (item->object_number == WILLARD_BOSS && CurrentLevel == 5 && item->item_flags[1] == 1)) && item->active && item->hit_points > 0)
		{
			bosshitpoints = item->hit_points;
			maxhitpoints = objects[item->object_number].hit_points;
			break;
		}
	}
	...
	if (bosshitpoints < 0)
		bosshitpoints = 0;
	else if (bosshitpoints > maxhitpoints)
		bosshitpoints = maxhitpoints;

	if (old_bosshitpoints != bosshitpoints)
	{
		old_bosshitpoints = bosshitpoints;
		boss_health_bar_timer = 40;
	}

	if (boss_health_bar_timer < 0)
		boss_health_bar_timer = 0;

	if (bosshitpoints > 0)
		S_DrawBossHealthBar(bosshitpoints * 100 / maxhitpoints);
}
...
void DrawHealthBar(long flash_state)
{
	...
	else if (health_bar_timer > 0 || hitpoints <= 0 || lara.gun_status == LG_READY || lara.poisoned >= 16)
		S_DrawHealthBar(hitpoints / 10);
...
void DrawGameInfo(long timed)
{
	...
	if (overlay_flag > 0)
	{
		...
		DrawBossHealthBar(flash_state);
		DrawAirBar(flash_state);
		//wc - custom UI		
		//gem x icon
		if (lara_item->ai_bits & 4)
			S_DrawScreenSprite2d(GetRenderScale(20), GetRenderScale(470), 0, GetRenderScale(0x10000) / 4, GetRenderScale(0x10000) / 4, short(objects[ALPHABET].mesh_index + 105), 0, 0);
		//gem icon
			S_DrawScreenSprite2d(GetRenderScale(20), GetRenderScale(470), 0, GetRenderScale(0x10000) / 6, GetRenderScale(0x10000) / 6, short(objects[ALPHABET].mesh_index + 103), 0, 0);
...
void AddDisplayPickup(short objnum)
{
	...
	if (objnum == PICKUP_ITEM2)
	{
		if (CurrentLevel == 4) //wc - pickup2 is grappling hook
		{
			S_Pause(9);
			if (lara_item->ai_bits & 1) //wolf form, so swap with meshswap3 instead of Lara
			{
				auto tmp = meshes[objects[MESHSWAP3].mesh_index + LARM_R];
				meshes[objects[MESHSWAP3].mesh_index + LARM_R] = meshes[objects[FLARE].mesh_index + LARM_R];
	...
	if (objnum == PICKUP_ITEM1)
	{
		if (CurrentLevel == 2) //wc - pickup1 in level 2 is aqua lung
		{
			S_Pause(8);
		...
		else if (CurrentLevel == 3) //wc - pickup1 in level 3 is moon shoes
		{
			S_Pause(7);
			lara_item->ai_bits |= 8;
			...
			for (int i : meshList)
			{
				if (lara_item->ai_bits & 1) //wolf form, so swap with meshswap3 instead of Lara
				{
					auto tmp = meshes[objects[MESHSWAP3].mesh_index + i];
					meshes[objects[MESHSWAP3].mesh_index + i] = meshes[objects[FLARE].mesh_index + i];
