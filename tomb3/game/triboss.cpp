...
static short FindLizardManItemNumber(short room_number, long x, long z)
{
	ITEM_INFO* item;

	short result = NO_ITEM;

	for (int i = 0; i < level_items; i++)
	{
		item = &items[i];

		//wc - use camera targets to update summon coordinates for lizards
		if (item->object_number == LIZARD_MAN && item->room_number == room_number)
			result = i;
		else
			if (item->object_number == CAMERA_TARGET && item->room_number == room_number)
			{
				long x2 = item->pos.x_pos - x;
				long z2 = item->pos.z_pos - z;
				short angle = short(phd_atan(z2, x2));
				int summonIndex = lizman_summon_coords[0][1] == -5376 ? 0 : 1;
				lizman_summon_coords[summonIndex][0] = item->pos.x_pos;
				lizman_summon_coords[summonIndex][1] = item->pos.y_pos;
				lizman_summon_coords[summonIndex][2] = item->pos.z_pos;
				if(angle > 0)
					lizman_summon_coords[summonIndex][3] = angle;
			}
		if (result != NO_ITEM && lizman_summon_coords[0][1] != -5376 && lizman_summon_coords[1][1] != -5376)
			break;
	}
...
static void ResetCollapsableFloors(short room_number, int y)
{
	ITEM_INFO* item;
	...
	for (int i = 0; i < level_items; i++)
	{
		...
		//wc - reset collapsable floors each time lizard man is summoned
		if (item->object_number == FALLING_BLOCK)
		{
			item->anim_number = objects[FALLING_BLOCK].anim_index;
			item->frame_number = anims[item->anim_number].frame_base;
			item->current_anim_state = 0;
			item->required_anim_state = 0;
			item->goal_anim_state = 0;
			item->pos.y_pos = y + (256 * 2);
			item->fallspeed = 0;
			item->gravity_status = 0;
			short new_room_number = item->room_number;
			GetFloor(item->pos.x_pos, item->pos.y_pos, item->pos.z_pos, &new_room_number);
			if (item->room_number != new_room_number)
				ItemNewRoom(i, new_room_number);
			RemoveActiveItem(i);
		}
	}
}
...
static void TribeBossDie(short item_number)
{
	...
	//wc - not killing so the platforms don't fall
...
void InitialiseTribeBoss(short item_number)
{
	...
	item = &items[item_number];
	bossdata.LizmanItem = FindLizardManItemNumber(item->room_number,item->pos.x_pos,item->pos.z_pos);
...
void TribeBossControl(short item_number)
{
	...
	{
		ResetCollapsableFloors(item->room_number, item->pos.y_pos);
		return;
	}
	...
	if (item->hit_points > 0)
	{
		...
		if (!bossdata.attack_flag)
		{
			...
			if (SQUARE(x) + SQUARE(z) < 0x400000 * 2) //wc - increase range to start attacking
	...
	{
		//wc - keep floor solid at end of fight
		ResetCollapsableFloors(item->room_number, item->pos.y_pos);
