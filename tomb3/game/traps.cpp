...
void LavaBurn(ITEM_INFO* item)
{
	...
	if (item->hit_points >= 0 && lara.water_status != LARA_CHEAT)
	{
		...
		if (item->floor == h)
		{
			if (GF_DeathTile == DEATH_RAPIDS || CurrentLevel == 5 || (CurrentLevel == 3 && (room[lara_item->room_number].flags & ROOM_NOT_INSIDE))) //wc - use rapids drown for desert level outdoors and finale
				LaraRapidsDrown();
			else
			{
				item->hit_status = 1;
				...
				if (GF_DeathTile == DEATH_ELECTRIC || (CurrentLevel == 3 && !(room[lara_item->room_number].flags & ROOM_NOT_INSIDE))) //wc - use electric death tiles indoors for desert level
					lara.electric = 1;
...
void ControlCeilingSpikes(short item_number)
{
	...
	if (TriggerActive(item) && item->status != ITEM_DEACTIVATED)
	{
		...
		{
			//wc - unlock fast travel if it was locked
