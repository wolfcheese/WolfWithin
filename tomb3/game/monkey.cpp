...
void InitialiseMonkey(short item_number)
{
	...
	item->anim_number = objects[MONKEY].anim_index + 31;
	item->frame_number = anims[item->anim_number].frame_base;
	item->current_anim_state = MONKEY_EMPTY;
	item->goal_anim_state = MONKEY_EMPTY;
	item->mesh_bits = 0;
...
void MonkeyControl(short item_number)
{
	...
	else
	{
		...
		if (item->current_anim_state != MONKEY_EMPTY)
		{
		...
		case MONKEY_EMPTY:
			...
			if (item->mesh_bits == 0x7FFFFFFF)
			{
				...
				effect_mesh = 0;
				monkey->flags = 0;
				item->hit_points = objects[MONKEY].hit_points;
			}
	...
	//wc - keep frozen monkey healthy and don't animate it until it's ready
	if (item->goal_anim_state == MONKEY_EMPTY)
	{
		item->hit_points = 900;
		return;
	}
...
void DrawMonkey(ITEM_INFO* item)
{
	...
	//wc - use "shiva" slot for frozen monkey, and other slot if it carries item (medpack)
	if (item->carried_item == NO_ITEM)
