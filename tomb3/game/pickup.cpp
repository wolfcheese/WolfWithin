...
static short PuzzleHoleBoundsUW[12] = { -512, 512, -1028, 1028, 0, 1028, -8190, 8190, -8190, 8190, -8190, 8190 };
...
void PuzzleHoleCollision(short item_number, ITEM_INFO* l, COLL_INFO* coll)
{
	...
	//wc - minesweeper puzzle hole in level 2
	if (CurrentLevel == 2 && item->object_number == PUZZLE_HOLE4 && l->current_anim_state == AS_STOP && l->anim_number == ANIM_BREATH)
	{
		short yRot = item->pos.y_rot;
		item->pos.y_rot = l->pos.y_rot;
		item->pos.z_rot = 0;
		item->pos.x_rot = 0;
		if (!(input & IN_ACTION) || lara.gun_status != LG_ARMLESS || !TestLaraPosition(PickUpBounds, &items[item_number], l))
		{
			item->pos.y_rot = yRot;
			return;
		}
		...
		item->pos.y_rot = yRot;
		//cannot remove hardcoded flags
		if (item->status == ITEM_ACTIVE && !item->ai_bits)
		{
	...
	else if (CurrentLevel == 2 && item->object_number == PUZZLE_HOLE4 && l->current_anim_state == AS_PICKUP && l->frame_number == anims[ANIM_PICKUP].frame_base + 42)
	{
		short yRot = item->pos.y_rot;
		item->pos.y_rot = l->pos.y_rot;
		item->pos.z_rot = 0;
		item->pos.x_rot = 0;
		if (TestLaraPosition(PickUpBounds, &items[item_number], l))
		{
			item->status = item->status == ITEM_INVISIBLE ? ITEM_ACTIVE : ITEM_INVISIBLE;
			if (item->status == ITEM_ACTIVE)
			{
				//ai bits track correctness of flag placement
				if (!item->ai_bits)
				{
					if ((item->flags & IFL_CODEBITS) == 512) //bit 1 is correct flag
						item->ai_bits = 1;
					else
						item->ai_bits = 2;
				}
		...
		item->pos.y_rot = yRot;
		//check for minesweeper solve
		ITEM_INFO* flagItem;
		bool solved = true;
		for (int i = 0; i < level_items && solved; i++)
		{
			flagItem = &items[i];

			if (flagItem->object_number == PUZZLE_HOLE4 && flagItem->room_number == item->room_number)
			{
				//ignore hardcoded flags
				if (flagItem->status == ITEM_ACTIVE && !flagItem->ai_bits)
				{
					continue;
				}
				bool correctFlag = (flagItem->flags & IFL_CODEBITS) == 512 || flagItem->ai_bits == 1;
				if (flagItem->status == ITEM_ACTIVE)
				{
					if (!correctFlag)
						solved = false;
				}
		...
		//trigger heavy trigger if solved
		if (solved) {
	...
	//wc - underwater puzzle hole in level 2
	if (CurrentLevel == 2 && (item->object_number == PUZZLE_HOLE3 || item->object_number == PUZZLE_HOLE1) && l->current_anim_state == AS_TREAD && l->anim_number == ANIM_TREAD)
	{
		if ((Inventory_Chosen == NO_ITEM && !(input & IN_ACTION)) || lara.gun_status != LG_ARMLESS || !TestLaraPosition(PuzzleHoleBoundsUW, &items[item_number], l))
			return;
		...
		{
			//wc - pre-select correct puzzle item
			for (int i = 0; i < inv_keys_objects; i++)
			{
				auto obj_num = Inv_GetItemOption(item->object_number - 4);
				auto item2 = inv_keys_list[i];

				if (item2->object_number == obj_num)
				{
					inv_keys_current = i;
					break;
				}
		...
		if (item->object_number == PUZZLE_HOLE3)
		{
		...
		if (item->object_number == PUZZLE_HOLE1)
		{
		...
		if (correct)
		{
			inv_keys_current = 0; //wc - reset this because the puzzle item was removed
			...
			l->goal_anim_state = AS_TREAD;
			l->pos.y_pos = item->pos.y_pos - 512;
			lara.gun_status = LG_HANDSBUSY;
			item->status = ITEM_ACTIVE;
			pup_x = l->pos.x_pos;
			pup_y = l->pos.y_pos;
	...
	else if (CurrentLevel == 2 && (item->object_number == PUZZLE_HOLE3 || item->object_number == PUZZLE_HOLE1) && l->current_anim_state == AS_PICKUP && TestLaraPosition(PuzzleHoleBoundsUW, &items[item_number], l) &&
		l->frame_number == anims[ANIM_PICKUP_UW].frame_base + 20)
	{
		if(item->object_number == PUZZLE_HOLE1)
	...
	else if (l->current_anim_state == AS_STOP && l->anim_number == ANIM_BREATH)
	{
