...
void FlamerControl(short item_number)
{
	...
	else
	{
		if (item->ai_bits)
			GetAITarget(flamer);
		else if (flamer->hurt_by_lara || CurrentLevel == 3) //wc - desert flamers are always hostile
		...
		if (flamer->enemy == lara_item)
		{
			...
			if (!flamer->hurt_by_lara && CurrentLevel != 3)
				flamer->enemy = 0;
		}
		else
		{
		...
		if (item->hit_status || (lara_info.distance < 0x100000 || TargetVisible(item, &lara_info)) && CurrentLevel == 3)
		{
			if (!flamer->alerted)
		...
		case 1:
			...
			if (item->ai_bits & GUARD)
			{
				...
				item->goal_anim_state = 2;
			else if (Targetable(item, &info) && (enemy != lara_item || flamer->hurt_by_lara || CurrentLevel == 3))
			{
		...
		case 4:
			...
			else if (Targetable(item, &info) && info.distance < 0x1000000 && (enemy != lara_item || flamer->hurt_by_lara || CurrentLevel == 3) ||
		...
		case 6:
			...
			if (info.ahead)
			{
				...
				if (Targetable(item, &info) && info.distance < 0x1000000 && (enemy != lara_item || flamer->hurt_by_lara || CurrentLevel == 3))
