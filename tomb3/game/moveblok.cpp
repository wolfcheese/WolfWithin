...
static short MovingBlockBoundsUW[12] = { -512, 512, -1028, 1028, -1028, -512, -8190, 8190, -8190, 8190, -8190, 8190 };
...
static long TestBlockPull(ITEM_INFO* item, long blockhite, ushort quadrant)
{
	...
	//wc - prevent pulling onto slanted floors
	int h1 = LaraFloorFront(lara_item, lara_item->pos.y_rot, -300);
	int h2 = LaraFloorFront(lara_item, lara_item->pos.y_rot, -1200);
	if (h1 != h2)
		return 0;
...
void MovableBlock(short item_number)
{
	...
	//wc - underwater bubbles
	if (item->item_flags[1] > 0)
	{
		PHD_3DPOS bubble;
		bubble.x_pos = (GetRandomControl() & 0x3F) + item->pos.x_pos - 32;
		bubble.y_pos = item->pos.y_pos - 1028 + (GetRandomControl() & 0xF);
		bubble.z_pos = (GetRandomControl() & 0x3F) + item->pos.z_pos - 32;
		room_number = item->room_number;
	...
	if (item->status == ITEM_DEACTIVATED)
	{
		...
		AlterFloorHeight(item, -WALL_SIZE);
		//wc - underwater block slides as far as it can
		if (item->item_flags[1] > 0 && TestBlockPush(item, WALL_SIZE, item->item_flags[1] - 10))
		{
			AlterFloorHeight(item, WALL_SIZE);
		...
		{
			//stop uw block and check if puzzle is complete
			if (item->item_flags[1] > 0)
			{
				item->item_flags[1] = 0;
				TriggerExplosionBubble(item->pos.x_pos, item->pos.y_pos - 1028, item->pos.z_pos, item->room_number);
				SoundEffect(SFX_LARA_THUD, &item->pos, SFX_WATER);

				int correctBlocks = 0;
				int targetPositions[3][2];
				int targetPositionIndex = 0;
				short temp_item_number;
				ITEM_INFO* temp_item;

				for (temp_item_number = room[item->room_number].item_number; temp_item_number != NO_ITEM; temp_item_number = temp_item->next_item)
				{
					temp_item = &items[temp_item_number];

					if (temp_item->object_number == CAMERA_TARGET)
					{
						targetPositions[targetPositionIndex][0] = temp_item->pos.x_pos;
						targetPositions[targetPositionIndex++][1] = temp_item->pos.z_pos;
					}
				}
				for (temp_item_number = room[item->room_number].item_number; temp_item_number != NO_ITEM; temp_item_number = temp_item->next_item)
				{
					temp_item = &items[temp_item_number];
					if (temp_item->object_number >= MOVABLE_BLOCK && temp_item->object_number <= MOVABLE_BLOCK3)
					{
						for (int i = 0; i < 3;i++)
						{
							if (targetPositions[i][0] == temp_item->pos.x_pos && targetPositions[i][1] == temp_item->pos.z_pos)
							{
								correctBlocks++;
							}
						}
				...
				{
					temp_item = &items[temp_item_number];
					if (temp_item->object_number == DOOR_TYPE8 && correctBlocks >= 3)
					{
						//if solved, activate heavy trigger
						room_number = item->room_number;
						floor = GetFloor(item->pos.x_pos, item->pos.y_pos, item->pos.z_pos, &room_number);
...
void MovableBlockCollision(short item_number, ITEM_INFO* l, COLL_INFO* coll)
{
	...
	if (!(input & IN_ACTION) || item->status == ITEM_ACTIVE || (l->gravity_status && CurrentLevel != 2) || (l->pos.y_pos != item->pos.y_pos && CurrentLevel != 2))
		return;
	...
	//wc - underwater pushblocks for deep sea
	else if (l->current_anim_state == AS_TREAD && CurrentLevel == 2)
	{
		if (lara.gun_status != LG_ARMLESS)
			return;
		...
		if (!TestLaraPosition(MovingBlockBoundsUW, item, l))
			return;
		...
		l->pos.y_rot = item->pos.y_rot;
		l->goal_anim_state = AS_KICK;
		do AnimateLara(l); while (l->current_anim_state != AS_KICK);

		if (l->current_anim_state == AS_KICK)
		{
			item->item_flags[1] = quadrant + 10;
