...
static void TriggerLaserBolt(PHD_VECTOR* pos, ITEM_INFO* item, long type, short ang)
{
	...
	{
		//wc - lowering y pos (otherwise bolt misses the player sliding down slope)
		phd_GetVectorAngles(lara_item->pos.x_pos - pos->x, lara_item->pos.y_pos - pos->y + 512, lara_item->pos.z_pos - pos->z, angles);
...
void InitialiseLondonBoss(short item_number)
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		{
			p->x = short((rad * rcossin_tbl[angle]) >> 11);
			p->y = (short)y;
			p->z = short((rad * rcossin_tbl[angle + 1]) >> 11);
			p->rgb = 0;
		}
	}
}
...
void LondonBossControl(short item_number)
{
	...
	//wc - make boss float in front of player
	...
	long yDiff = item->hit_points <= 0 ? 0 : item->pos.y_pos - h;
	item->pos.z_pos = lara_item->pos.z_pos - (256 * 8);
	floor = GetFloor(item->pos.x_pos, item->pos.y_pos, item->pos.z_pos, &room_number);
	h = GetHeight(floor, item->pos.x_pos, item->pos.y_pos, item->pos.z_pos);
	item->pos.y_pos = h + yDiff;
	if (item->room_number != room_number)
		ItemNewRoom(item_number, room_number);
			...
			if (!bossdata.explode_count)
			{
				...
				if (!bossdata.dropped_icon)
				{
					...
					for (int i = 0; i < level_items; i++)
					{
						...
						if (item2->object_number == ICON_PICKUP3_ITEM)
						{
							AddActiveItem(i);
							break;
						}
	...
	else
	{
		...
		//wc - do not make this boss invincible
		if (item->hit_points > 350)
			item->hit_points = 350;
		...
		case LONDONBOSS_SUMMON:
			head = lara_info.angle;
			TribeBossShieldOn = 1; //wc - shielded during charge
			if (sophia->reached_goal)
			{
				sophia->reached_goal = 0;
				item->ai_bits = AMBUSH;
	...
	AnimateItem(item); //wc - this avoids collision issues from CreatureAnimation() since this boss just floats
	/* wc - creaturevault snaps height, I don't want that
	else
	{
