...
void lara_as_swim(ITEM_INFO* item, COLL_INFO* coll)
{
	...
	{
		//wc - wolf form has swim swprint
		bool sprinting = input & IN_SPRINT && lara_item->ai_bits & 1;
		SwimTurn(item);
		item->fallspeed += sprinting ? 16 : 8;
		...
		else if (CurrentLevel == 3 && item->fallspeed > 100 && !(room[lara_item->room_number].flags & ROOM_OUTSIDE)) //wc - viscous goo in desert is harder to swim through
			item->fallspeed = 100;
		else if (item->fallspeed > 200 && !sprinting)
			item->fallspeed = 200;
		else if (item->fallspeed > 325)
			item->fallspeed = 325;
		if (!(input & IN_JUMP))
			item->goal_anim_state = AS_GLIDE;
		else if (sprinting && item->anim_number - objects[item->object_number].anim_index < 300)
			item->goal_anim_state = AS_KICK;
		else if (!sprinting && item->anim_number - objects[item->object_number].anim_index > 300)
			item->goal_anim_state = AS_KICK;
		else item->goal_anim_state = AS_SWIM;
	}
}
