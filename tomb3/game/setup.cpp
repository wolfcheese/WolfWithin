		...
		if (objects[item->object_number].intelligent)
		{
			for (ai_item_number = room[item->room_number].item_number; ai_item_number != NO_ITEM; ai_item_number = ai_item->next_item)
			{
					...
					objects[ai_item->object_number].collision == AIPickupCollision && ai_item->object_number < AI_PATROL2)
				{
					...
					if (!(ai_item->object_number == AI_PATROL1 && (CurrentLevel == 3)))
					{
...
long InitialiseLevel(long level, long type)
{
	...
	//wc - recall completed savegame info if revisiting. Then snap back Lara to starting point
	if (type == 1 && ((savegame.IndiaComplete && level == 1) 
		|| (savegame.SPacificComplete && level == 2) 
		|| (savegame.NevadaComplete && level == 3) 
		|| (savegame.LondonComplete && level == 4)))
	{
		SAVEGAME_INFO tempSaveGame = savegame;
		long laraX = lara_item->pos.x_pos;
		long laraY = lara_item->pos.y_pos;
		long laraZ = lara_item->pos.z_pos;
		long laraRot = lara_item->pos.y_rot;
		short laraRoomNumber = lara_item->room_number;
		S_LoadGame(&savegame, sizeof(SAVEGAME_INFO), 30 + level);
		ExtractSaveGameInfo();
		//remember the warp gems
		tempSaveGame.num_key1 = savegame.num_key1;
		tempSaveGame.num_key2 = savegame.num_key2;
		tempSaveGame.num_key3 = savegame.num_key3;
		tempSaveGame.num_key4 = savegame.num_key4;
		...
		savegame.timer = savegame.start[level].timer;
		savegame.ammo_used = savegame.start[level].ammo_used;
		savegame.ammo_hit = savegame.start[level].ammo_hit;
		savegame.distance_travelled = savegame.start[level].distance_travelled;
		savegame.kills = savegame.start[level].kills;
		savegame.secrets = savegame.start[level].secrets_found;
		savegame.health_used = savegame.start[level].health_used;
		lara_item->pos.x_pos = laraX;
		lara_item->pos.y_pos = laraY;
		lara_item->pos.z_pos = laraZ;
		lara_item->pos.y_rot = laraRot;
		if (lara_item->room_number != laraRoomNumber)
			ItemNewRoom(lara.item_number, laraRoomNumber);
	}
	else
	if (type == 2)
		ExtractSaveGameInfo();
	else
	{
		...
		//wc - add puzzle items for finale fight
		if (level == 5)
		{
	...
	//swap in "upgrade" human meshes (and set bits) based on player's progress
	if (lara.item_number != NO_ITEM && (savegame.NevadaComplete || savegame.bonus_flag || (lara_item->ai_bits & 8)))
	{
		lara_item->ai_bits |= 8;
		int meshList[] = { FOOT_L,FOOT_R,CALF_L,CALF_R };
		for (int i: meshList)
		{
			auto tmp = meshes[objects[LARA].mesh_index + i];
			meshes[objects[LARA].mesh_index + i] = meshes[objects[FLARE].mesh_index + i];
			lara.mesh_ptrs[i] = meshes[objects[FLARE].mesh_index + i];
			meshes[objects[FLARE].mesh_index + i] = tmp;
		}
	}
	if (lara.item_number != NO_ITEM && (savegame.SPacificComplete || savegame.bonus_flag || (lara_item->ai_bits & 16)))
	{
		lara_item->ai_bits |= 16;
		lara_item->item_flags[3] = 0;
		if(lara.water_status == LARA_UNDERWATER || lara.water_status == LARA_SURFACE)
			ToggleAquaLungMesh(true);
		else
			ToggleAquaLungMesh(false);
	}
	INVENTORY_ITEM* invitem;
	bool hasHook = Inv_RequestItem(PICKUP_ITEM2);
	if (lara.item_number != NO_ITEM && (savegame.LondonComplete || savegame.bonus_flag || hasHook))
	{
		if (!hasHook)
			Inv_AddItem(PICKUP_ITEM2);
		auto tmp = meshes[objects[LARA].mesh_index + LARM_R];
		meshes[objects[LARA].mesh_index + LARM_R] = meshes[objects[FLARE].mesh_index + LARM_R];
		lara.mesh_ptrs[LARM_R] = meshes[objects[FLARE].mesh_index + LARM_R];
		meshes[objects[FLARE].mesh_index + LARM_R] = tmp;
	}

	//wc - initialize wolf skin if in wolf form (then, swap in weapon/minecart hands if needed)
	if (lara.item_number != NO_ITEM && lara_item->ai_bits & 1)
	{
		swap_meshes_with_meshswap3(lara_item);
		lara_item->ai_bits ^= 1;
		if (lara.gun_status == LG_READY || lara.gun_status == LG_SPECIAL || lara.gun_status == LG_DRAW || lara.gun_status == LG_UNDRAW)
		{
			long obj;
			obj = WeaponObject(lara.gun_type);
			lara.mesh_ptrs[HAND_R] = meshes[objects[obj].mesh_index + HAND_R];
			if (lara.gun_type == LG_UZIS || lara.gun_type == LG_PISTOLS)
			{
				lara.mesh_ptrs[HAND_L] = meshes[objects[obj].mesh_index + HAND_L];
			}
		}
		if (lara.skidoo != NO_ITEM && items[lara.skidoo].object_number == MINECART)
		{
	...
	//wc - after first level (or adventure complete), unlock transformation
	if (lara.item_number != NO_ITEM && (savegame.IndiaComplete || savegame.bonus_flag))
		lara_item->ai_bits |= 2;
...
static void BaddyObjects()
{
	...
	if (obj->loaded)
	{
		...
		obj->save_flags = 1;
		obj->save_anim = 1;
		obj->draw_routine = S_DrawTrexBoss;
	...
	if (obj->loaded)
	{
		//wc - using shiva slot for shape puzzle in desert
		obj->initialise = InitialiseShiva;
		obj->collision = ObjectCollision;
		obj->hit_points = 0;
	}

	obj = &objects[MAP_CLOSED];

	if (obj->loaded)
	{
		//wc - using this slot for laser puzzle in desert
		obj->collision = ObjectCollision;
		obj->hit_points = 0;
		obj->save_position = 1;
		obj->save_flags = 1;
