...
void lara_col_duck(ITEM_INFO* item, COLL_INFO* coll)
{
	...
	else
	{
		...
		//wc - crouch jump (human upgrade)
		if (input & IN_JUMP && (item->ai_bits & 1) == 0 && item->ai_bits & 8 && coll->mid_ceiling < -362)
		{
			item->goal_anim_state = AS_KICK;
		}
	}
}
...
void lara_col_all4s(ITEM_INFO* item, COLL_INFO* coll)
{
	...
	if (item->goal_anim_state == AS_CRAWL2HANG || item->goal_anim_state == AS_KICK)
		return;
	...
	if (input & IN_JUMP)
	{
		h = LaraCeilingFront(item, item->pos.y_rot, 100);

		if (h != NO_HEIGHT && h <= 256)
		{
			h = LaraCeilingFront(item, item->pos.y_rot, 900);
			if (h != NO_HEIGHT)
			{
				h = LaraFloorFront(item, item->pos.y_rot, 300);
				if (h > 255 * 2 && !GetStaticObjects(item, item->pos.y_rot + 0x8000, 512, 50, 300))
				{
					item->goal_anim_state = AS_KICK;
					return;
				}
...
void lara_col_crawl2hang(ITEM_INFO* item, COLL_INFO* coll)
{
	...
	/*if (TestHangSwingIn(item, angle)) wc - swinging ledge grab won't respond to controls, removing it
	{
		lara.head_x_rot = 0;
		lara.head_y_rot = 0;
		lara.torso_x_rot = 0;
...
void MonkeySwingSnap(ITEM_INFO* item, COLL_INFO* coll)
{
	...
	long newYPos = GetCeiling(floor, item->pos.x_pos, item->pos.y_pos, item->pos.z_pos) + 704;
	if (newYPos - item->pos.y_pos < -2048) //wc - player lets go if shift is greater than 8 clicks
		MonkeySwingFall(item);
	else
		item->pos.y_pos = newYPos;
}
...
void lara_col_hang2(ITEM_INFO* item, COLL_INFO* coll)
{
	if (lara.CanMonkeySwing)
	{
		...
		if (input & IN_ROLL || input & IN_BACK) //wc - add monkeyswing turnaround
			item->goal_anim_state = AS_KICK;
		else if (input & IN_FORWARD && coll->coll_type != CT_FRONT && abs(coll->mid_ceiling - coll->front_ceiling) < 50)
	...
	else
	{
			...
			coll->left_floor2 - coll->left_ceiling2 >= 0 && coll->right_floor2 - coll->right_ceiling2 >= 0 && !coll->hit_static)
		{
			...
			//wc - faster climb as human than wolf
			else if ((item->ai_bits & 1) == 0)
				item->goal_anim_state = AS_KICK;
			else
...
void lara_as_stop(ITEM_INFO* item, COLL_INFO* coll)
{
	...
	//wc - transform between human and wolf form
	//the second ai_bit determins if transforming is available or not
	if (lara.gun_status == LG_ARMLESS && input & IN_SPRINT && lara_item->ai_bits & 2)
	{
		item->goal_anim_state = AS_KICK;
		lara.gun_status = LG_HANDSBUSY;
		return;
...
void lara_as_deathslide(ITEM_INFO* item, COLL_INFO* coll)
{
	...
	room_number = item->room_number;
	floor = GetFloor(item->pos.x_pos, item->pos.y_pos, item->pos.z_pos, &room_number);
	long h = GetHeight(floor, item->pos.x_pos, item->pos.y_pos, item->pos.z_pos);
	long c = GetCeiling(floor, item->pos.x_pos, item->pos.y_pos, item->pos.z_pos);
	coll->trigger = trigger_index;

	//wc - grappling hook sliding is handled here
	if (item->anim_number == 321)
	{
		lara_item->gravity_status = 1;
		lara_item->fallspeed = 0;
		if (item->frame_number >= anims[item->anim_number + objects[LARA].anim_index].frame_end)
			item->speed = 100;
		return;
	}
	else
	if (item->anim_number == 322)
	{
		item->fallspeed = 0;
		item->gravity_status = 1;
		lara.move_angle = item->pos.y_rot;
		...
		GetLaraCollisionInfo(item, coll);
		h = LaraFloorFront(item, item->pos.y_rot, 512);
		//use the "deflectedgejump" test to determine we've reached the wall or the ground
		if ((!lara.climb_status || item->speed != 2) && (h == NO_HEIGHT || h < -10))
		{
			if (item->pos.y_rot >= -6370 && item->pos.y_rot <= 6370)
				item->pos.y_rot = 0;
			else if (item->pos.y_rot >= 10014 && item->pos.y_rot <= 22754)
				item->pos.y_rot = 0x4000;
			else if (item->pos.y_rot >= 26397 || item->pos.y_rot <= -26397)
				item->pos.y_rot = -0x8000;
			else if (item->pos.y_rot >= -22754 && item->pos.y_rot <= -10014)
				item->pos.y_rot = -0x4000;
			...
			lara.gun_status = LG_ARMLESS;
			lara_item->goal_anim_state = AS_REACH;
			lara_item->current_anim_state = AS_REACH;
			lara_item->speed = 20;
