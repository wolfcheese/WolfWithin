...
void ModifyStartInfo(long level)
{
	...
	{
		//wc - the artifacts collected are turned into puzzle items for the finale
		pInfo->num_icon1 = 0;
		pInfo->num_icon2 = 0;
		pInfo->num_icon3 = 0;
		pInfo->num_icon4 = 0;
...
void InitialiseStartInfo()
{
	...
	savegame.IndiaComplete = 0; //WC - TEST ONLY PLEASE REVERT
	...
	savegame.bonus_flag = 0; //WC - TEST ONLY PLEASE REVERT
...
void CreateSaveGameInfo()
{
	...
	for (int i = 0; i < level_items; i++)
	{
		...
		if (obj->save_position || (objnum == TRAPDOOR && savegame.current_level == 4))  //wc - save position of trapdoors in tundra (for valve puzzle)
		{
		...
		//wc - save meshbits of monkeys and image puzzle
		if (objnum == MONKEY || objnum == SHIVA)
		{
...
void ExtractSaveGameInfo()
{
	...
	for (int i = 0; i < level_items; i++)
	{
		...
		if (obj->save_flags)
		{
			...
			{
				//wc - make sure minesweeper puzzle is unaffected
				if (!(CurrentLevel == 2 && item->object_number == PUZZLE_HOLE4))
				{
		...
		//wc - track horizon swap for desert boss
		if (item->object_number == TONY)
		{
			if (item->hit_points > 0 && item->active && item->item_flags[2] > 0)
			{
				auto obj = &objects[HORIZON];
				auto tmp = meshes[obj->mesh_index];
				meshes[obj->mesh_index] = meshes[obj->mesh_index + 1];
				meshes[obj->mesh_index + 1] = tmp;
			}
		}
	}
