...
void ToggleAquaLungMesh(bool visible)
{
	//wc - toggle aqua lung if upgrade was acquired, and currently in human form
	if (lara_item->ai_bits & 16 && ((lara_item->ai_bits & 1) == 0))
	{
		if ((lara_item->item_flags[3] && !visible) || (!lara_item->item_flags[3] && visible))
		{
			lara_item->item_flags[3] = visible ? 1 : 0;
			auto tmp = meshes[objects[LARA].mesh_index + HEAD];
			meshes[objects[LARA].mesh_index + HEAD] = meshes[objects[FLARE].mesh_index + HEAD];
			lara.mesh_ptrs[HEAD] = meshes[objects[FLARE].mesh_index + HEAD];
			meshes[objects[FLARE].mesh_index + HEAD] = tmp;
		}
	}
}
...
#endif
	{
		if (lara.skidoo == NO_ITEM)
		{
			...
			if (lara.water_status != LARA_CHEAT)
			{
				...
				lara.head_x_rot = 0;
				lara.head_y_rot = 0;
				swap_meshes_with_meshswap3(lara_item); //wc - use cheat to toggle human/wolf
			}
		}
	}
...
void LaraControl(short item_number)
{
	...
	static long exposure_state = 0, exposure_count = 0;
	...
	case LARA_WADE:
		...
		if (CurrentLevel == 4)
		{
			if (lara.water_status == LARA_WADE && room[lara_item->room_number].flags & ROOM_OUTSIDE) //wc - only do exposure for outdoor rooms
			{
				if (exposure_state || (lara_item->ai_bits & 1) == 0) //wc - wolf exposure is slower
					ExposureMeter--;
			}
			else 
			{
	...
	case LARA_UNDERWATER:
		...
		if(lara_item->hit_points >= 0)
		{
			if (!((lara_item->ai_bits & 16) && (lara_item->ai_bits & 1) == 0 && lara.air < 1800)) // wc - aqua lung upgrade does not deplete air
			{
				if (CurrentLevel == 3 && !(room[lara_item->room_number].flags & ROOM_OUTSIDE)) //wc - interior water in desert is viscous goo, faster air depletion
					lara.air -= 15;
				else
					lara.air--;
			}
		...
		if (CurrentLevel == 4 && room[lara_item->room_number].flags & ROOM_OUTSIDE) //wc - only do exposure for outdoor rooms
		{
			if (exposure_state || (lara_item->ai_bits & 1) == 0) //wc - wolf exposure is slower
				ExposureMeter -= 2;
		}
