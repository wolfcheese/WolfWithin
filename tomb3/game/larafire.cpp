...
long FireWeapon(long weapon_type, ITEM_INFO* target, ITEM_INFO* source, short* angles)
{
	...
	if (best < 0)
	{
		...
		else if (items[objLos].object_number == SMASH_OBJECT1 && (CurrentLevel == 4 || CurrentLevel == 2))
			Richochet(&dest);
		else
	...
	if (objLos != NO_ITEM && (items[objLos].object_number != SMASH_OBJECT1 || (CurrentLevel != 4 && CurrentLevel != 2)))
		SmashItem((short)objLos, weapon_type);

	obj_num = target->object_number;

	if (obj_num != SHIVA && obj_num != ARMY_WINSTON && (obj_num != LON_BOSS || !TribeBossShieldOn) && (obj_num != TRIBEBOSS || !TribeBossShieldOn))
		HitTarget(target, &dest, wep->damage);
	else if (obj_num == TRIBEBOSS || obj_num == LON_BOSS) //wc - tundra boss can use shield
...
void HitTarget(ITEM_INFO* item, GAME_VECTOR* hitpos, long damage)
{
	...
	{
		savegame.kills++;
		//wc - target triggers heavy when "killed"
...
void LaraGun()
{
	...
	case LG_READY:
		...
		//wc - remove angry face swap logic
