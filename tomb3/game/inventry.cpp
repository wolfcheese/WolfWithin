...
{
	//wc - customizing globe points for my adventure
	GlobeLevelComplete[0] = savegame.IndiaComplete && CurrentLevel == 1 ? 1 : 0;
	GlobeLevelComplete[1] = savegame.SPacificComplete && CurrentLevel == 2 ? 1 : 0;
	GlobeLevelComplete[2] = savegame.NevadaComplete && CurrentLevel == 3 ? 1 : 0;
	GlobeLevelComplete[3] = -1;
	GlobeLevelComplete[4] = savegame.IndiaComplete && savegame.SPacificComplete && savegame.NevadaComplete && savegame.LondonComplete ? 0 : 1;
	GlobeLevelComplete[5] = savegame.LondonComplete && CurrentLevel == 4 ? 1 : 0;
...
void Construct_Inventory()
{
	...
	if (Inventory_Mode == INV_TITLE_MODE)
		inv_option_objects = 4; //wc - remove lara's home option
	else
	...
	inv_main_current = 0;
	OpenInvOnGym = 0; //wc - disable Lara's Home
...
long Display_Inventory(long mode)
{
	...
	case INV_KEYS_MODE:
		Inv_RingInit(&ring, 2, inv_keys_list, inv_keys_objects, inv_keys_current, &imo);
		break;
