...
void DrawBoat(ITEM_INFO* item)
{
	...
	item->data = boat;
	//wc - only draw wake if using this boat
	if(&items[lara.skidoo] == item)
		S_DrawWakeFX(&items[lara.skidoo]);
}
...
static long BoatUserControl(ITEM_INFO* item)
{
	...
	{
		//wc - put this in case player strands boat in snow
		if (input & IN_ROLL)
		{
			if (input & (IN_RSTEP | IN_RIGHT))
...
static void BoatAnimation(ITEM_INFO* item, long collide)
{
	...
	else if (item->pos.y_pos < boat->water - 128 && item->fallspeed > 0)
	{
		...
		{
			//wc - fix issues coming off of land
